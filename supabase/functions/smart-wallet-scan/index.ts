...
const reply=(body:unknown,status=200)=>new Response(JSON.stringify(body),{status,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*','Access-Control-Allow-Headers':'authorization, x-client-info, apikey, content-type, x-paper-internal-token','Cache-Control':'public, max-age=10, s-maxage=20, stale-while-revalidate=45'}})
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    let subject=''
    if(publicGet){
      const rawIp=req.headers.get('cf-connecting-ip')||req.headers.get('x-forwarded-for')||req.headers.get('x-real-ip')||'unknown'
      subject='public:'+rawIp.split(',')[0].trim().slice(0,80)
      const {data:rate}=await admin.rpc('paper_consume_server_rate_limit_v1',{p_scope:'smart-wallet-public',p_subject:subject,p_limit:20,p_window_seconds:60})
    ...
    const body=publicGet?{}:await req.json().catch(()=>({}))
    const address=String(publicGet?new URL(req.url).searchParams.get('address')||'':body?.address||'').trim()
