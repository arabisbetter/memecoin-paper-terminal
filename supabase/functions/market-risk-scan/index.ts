...
async function rpcFallback(endpoints:{url:string;name:string}[],method:string,params:unknown[]){
  ...
  for(const endpoint of endpoints){
    try{
      const value=await rpc(endpoint.url,method,params)
      return{...value,provider:endpoint.name}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(!heliusKey)await health(admin,'helius',false,null,'Optional in free mode; no HELIUS_API_KEY configured',{skipped:true,optional:true,mode:'free'})
    const rpcEndpoints=[
      ...(heliusEndpoint?[{url:heliusEndpoint,name:'helius'}]:[]),
    ...
    try{
      const mintRes=await rpcFallback(rpcEndpoints,'getAccountInfo',[mint,{encoding:'jsonParsed',commitment:'confirmed'}])
      const preferred=[...rpcEndpoints.filter(x=>x.name===mintRes.provider),...rpcEndpoints.filter(x=>x.name!==mintRes.provider)]
      const largestRes=await rpcFallback(preferred,'getTokenLargestAccounts',[mint,{commitment:'confirmed'}])
      ...
      let ownerProvider=largestRes.provider
      if(tokenAccounts.length){
        const ownersRes=await rpcFallback(preferred,'getMultipleAccounts',[tokenAccounts,{encoding:'jsonParsed',commitment:'confirmed'}])
        ownerProvider=ownersRes.provider
      ...
      const used=[mintRes.provider,largestRes.provider,ownerProvider]
      const usedHelius=used.includes('helius')
      const usedFree=used.some(x=>x!=='helius')
      if(usedHelius){sources.push('helius');await health(admin,'helius',true,chainLatency)}
      if(usedFree||!heliusKey){
        sources.push('solana_public_rpc')
        await health(admin,'solana_public_rpc',true,chainLatency,undefined,{endpoints:[...new Set(used.filter(x=>x!=='helius'))],mode:'free'})
      }
    }catch(e){
      await health(admin,'solana_public_rpc',false,chainLatency||null,e instanceof Error?e.message:String(e),{mode:'free'})
      if(heliusKey)await health(admin,'helius',false,chainLatency||null,e instanceof Error?e.message:String(e))
    }
