...
import { Turnkey } from 'npm:@turnkey/sdk-server@8.4.0'
import { TurnkeySigner } from 'npm:@turnkey/solana@1.1.40'
...
function rpcUrl(){
  const configured=Deno.env.get('SOLANA_RPC_URL')
...
async function solUsd(){
  try{
    const r=await fetch(`https://api.dexscreener.com/tokens/v1/solana/${WSOL}`,{headers:{Accept:'application/json'}})
    if(!r.ok)return 0
...
async function jupiterOrder(params:{inputMint:string;outputMint:string;amount:string;taker:string;slippageBps:number}){
  ...
  // Force Jupiter's Metis router; external RFQ/aggregator routers are excluded.
...
function routeLabels(order:any){
  const labels=(Array.isArray(order?.routePlan)?order.routePlan:[])
    .map((x:any)=>String(x?.swapInfo?.label||x?.label||'').trim()).filter(Boolean)
  return [...new Set(labels)]
}
function assertPumpOnly(labels:string[]){
  if(!labels.length)throw new Error('ROUTE_LABELS_UNAVAILABLE')
  if(labels.some(x=>!/pump/i.test(x)))throw new Error('VENUE_NOT_ALLOWED')
}
async function jupiterExecute(signedTransaction:string,requestId:string,lastValidBlockHeight:unknown){
  const key=Deno.env.get('JUPITER_API_KEY')
  if(!key)throw new Error('JUPITER_API_KEY_NOT_CONFIGURED')
...
function turnkeySigner(){
  const apiPublicKey=Deno.env.get('TURNKEY_API_PUBLIC_KEY'),apiPrivateKey=Deno.env.get('TURNKEY_API_PRIVATE_KEY')
  const organizationId=Deno.env.get('TURNKEY_ORGANIZATION_ID'),policyId=Deno.env.get('TURNKEY_POLICY_ID')
...
async function atomicAmount(value:number,decimals:number){
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const mint=String(body?.mint||'').trim(),side=String(body?.side||'').toLowerCase()
    ...
    if(!['buy','sell'].includes(side))return reply({error:'side must be buy or sell'},400)
    if(!idempotencyKey||idempotencyKey.length>128)return reply({error:'idempotencyKey required'},400)
    if(slippageBps<1||slippageBps>500)return reply({error:'slippage must be between 1 and 500 bps'},400)
    ...
    if(existing){
      return reply({ok:existing.status==='confirmed',idempotent:true,orderId:existing.id,status:existing.status,txSignature:existing.tx_signature||null,rejectionReason:existing.rejection_reason||existing.last_error||null})
    }
    ...
    if(!account)return reply({error:'FUNDED_ACCOUNT_NOT_ACTIVE'},409)
    ...
    if(!custody?.wallet_account_address)return reply({error:'CUSTODY_WALLET_NOT_ACTIVE'},409)

    let notionalUsd=finite(body?.notionalUsd),sellPct=finite(body?.sellPct,100),sellQty=0,decimals=6
    if(side==='buy'){
      if(notionalUsd<=0)return reply({error:'notionalUsd required for buy'},400)
      await refreshRisk(url,auth,mint)
    }else{
      if(sellPct<=0||sellPct>100)return reply({error:'sellPct must be >0 and <=100'},400)
      const {data:position,error:positionError}=await admin.from('paper_funded_positions').select('*')
        .eq('funded_account_id',account.id).eq('token_address',mint).eq('status','open').maybeSingle()
      if(positionError)throw new Error(positionError.message)
      if(!position||finite(position.quantity_tokens)<=0)return reply({error:'NO_OPEN_FUNDED_POSITION'},409)
      ...
      if(!mark||mark.data_status!=='LIVE'||new Date(mark.observed_at).getTime()<Date.now()-90_000)return reply({error:'FRESH_MARK_REQUIRED'},409)
    ...
    if(preflightError)return reply({error:preflightError.message},409)

    const {policyId,organizationId,signer}=turnkeySigner()
    if(custody.policy_ref!==policyId)return reply({error:'CUSTODY_POLICY_MISMATCH'},409)
    if(String(custody.organization_id||'')!==organizationId)return reply({error:'CUSTODY_ORGANIZATION_MISMATCH'},409)

    decimals=side==='buy'?6:await tokenDecimals(mint)
    const inputMint=side==='buy'?USDC:mint,outputMint=side==='buy'?mint:USDC
    const amount=side==='buy'?await atomicAmount(notionalUsd,6):await atomicAmount(sellQty,decimals)
    const order=await jupiterOrder({inputMint,outputMint,amount,taker:custody.wallet_account_address,slippageBps})
    const labels=routeLabels(order)
    assertPumpOnly(labels)
    ...
    const {data:created,error:createError}=await admin.from('paper_funded_orders').insert({
      ...
      requested_notional_usd:notionalUsd,status:'quoted',idempotency_key:idempotencyKey,
      input_mint:inputMint,output_mint:outputMint,slippage_bps:slippageBps,
      protocol_fee_bps:Number(preflight?.protocol_fee_bps||100),quote_provider:'jupiter_v2',
      route_labels:labels,price_impact_pct:finite(order?.priceImpactPct),
      expected_out_amount_atomic:String(order?.outAmount||order?.outputAmount||''),
    ...
    await admin.from('paper_funded_execution_events').insert({
      order_id:orderId,funded_account_id:account.id,user_id:userId,event_type:'signed',provider:'turnkey',
      details:{policyRef:policyId,routeLabels:labels}
    })

    const executed=await jupiterExecute(signedB64,String(order.requestId),order.lastValidBlockHeight)
    const signature=String(executed.signature),slot=Math.round(finite(executed.slot))
    await admin.from('paper_funded_orders').update({
      status:'broadcast',tx_signature:signature,broadcast_at:new Date().toISOString(),updated_at:new Date().toISOString()
    }).eq('id',orderId)
    ...
    const qty=side==='buy'?Number(outputAtomic)/10**(await tokenDecimals(mint)):Number(inputAtomic)/10**decimals
    ...
    const fillPrice=usd/qty
    const feeLamports=await actualFeeLamports(signature)
    const solPrice=feeLamports>0?await solUsd():0
    ...
    await admin.from('paper_funded_orders').update({
      actual_out_amount_atomic:String(outputAtomic),network_fee_lamports:feeLamports,updated_at:new Date().toISOString()
    }).eq('id',orderId)

    return reply({ok:true,orderId,status:'confirmed',txSignature:signature,slot,side,mint,quantityTokens:qty,notionalUsd:usd,fillPriceUsd:fillPrice,protocolFeeUsd:finite(fill?.protocol_fee_usd),networkFeeUsd,routeLabels:labels})
  }catch(error){
