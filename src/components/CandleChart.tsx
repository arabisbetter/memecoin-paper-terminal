...
export default function CandleChart({ poolAddress, currentPrice }: { poolAddress?: string; currentPrice?: number }) {
  ...
  useEffect(() => {
    ...
    async function load(force=false) {
      ...
      try {
        ...
        if (alive && Array.isArray(j.candles) && j.candles.length) { setCandles(j.candles); setError('') }
  ...
  return <div className="chart-card">
    <div className="chart-toolbar"><div><span className="chart-title">REAL OHLCV</span>{hoverCandle && <span className="ohlc-readout">O {formatPrice(hoverCandle.open)} · H {formatPrice(hoverCandle.high)} · L {formatPrice(hoverCandle.low)} · C {formatPrice(hoverCandle.close)}</span>}</div><div className="tf-group">{(['1m','5m','15m','1h'] as Timeframe[]).map(v => <button key={v} className={tf===v?'tf active':'tf'} onClick={()=>setTf(v)}>{v}</button>)}</div></div>
    <div className="chart-stage" onMouseLeave={()=>setHovered(null)}>
      ...
      {poolAddress && !visible.length && !error && <div className="chart-state">Loading real market candles…</div>}
