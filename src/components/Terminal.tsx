...
import { Activity, Copy, Eye, Globe2, Info, MessageCircle, Search, ShieldCheck, Zap } from 'lucide-react'
...
export default function Terminal(){
  ...
  const [presetEditorOpen,setPresetEditorOpen]=useState(false),[presetSaving,setPresetSaving]=useState(false),[presetDraft,setPresetDraft]=useState<PaperPresetValues>([0.1,0.5,1,5]),[advancedOpen,setAdvancedOpen]=useState(false)
  ...
  useEffect(()=>{
    const apply=(event:Event)=>{
      ...
      if(value>0){setAmount(value);localStorage.setItem('paper.quickBuySize',String(value));setMobilePane('trade')}
    }
  ...
  useEffect(()=>{localStorage.setItem('paper.maxSlippagePct',String(maxSlippagePct))},[maxSlippagePct])
  ...
  async function persistPresets(){
    ...
    try{
      ...
      const activeId=(localStorage.getItem('paper.quickBuyPreset')||selectedPresetId)
      const index=Math.max(0,Math.min(3,Number(activeId.slice(1))-1))
      const activeValue=Number(saved[index]||saved[0])
      localStorage.setItem('paper.quickBuySize',String(activeValue))
      setPresetEditorOpen(false);setAmount(activeValue);setMessage('Quick-buy presets saved.')
    }
  ...
  useEffect(()=>{
    const handler=(event:KeyboardEvent)=>{
      ...
      if(['1','2','3','4'].includes(key)){
        ...
        if(side==='buy')setAmount(paperPresets[index])
  ...
  },[side,busy,selected?.mint,sellPct,canBuy,paperPresets])
  ...
  return <div className="ax-app"><AppHeader active="spot"/><nav className="mobile-terminal-tabs"><button className={mobilePane==='chart'?'active':''} onClick={()=>setMobilePane('chart')}>Chart</button><button className={mobilePane==='trade'?'active':''} onClick={()=>setMobilePane('trade')}>Trade</button><button className={mobilePane==='positions'?'active':''} onClick={()=>setMobilePane('positions')}>Positions</button><button className={mobilePane==='info'?'active':''} onClick={()=>setMobilePane('info')}>Info</button></nav><main data-mobile-pane={mobilePane} className="spot-detail phase1-terminal parts23-terminal" style={{'--paper-token-rail':tokenRailWidth+'px','--paper-trade-panel':tradePanelWidth+'px'} as CSSProperties}>
    ...
    <button className="paper-panel-resizer trade-resizer" aria-label="Resize trade panel" onPointerDown={e=>beginPanelResize('trade',e)}/><aside className="spot-trade-panel axiom-paper-trade p23-trade-panel"><div className="trade-window-tabs">{(['1m','5m','1h','6h','24h'] as StatsWindow[]).map(w=><button key={w} className={statsWindow===w?'active':''} onClick={()=>setStatsWindow(w)}>{w}</button>)}</div><div className="trade-stats-top"><span><small>{activeStats.label} Vol</small><b>{activeStats.ready?money(activeStats.volume):'warming'}</b></span><span><small>Buys</small><b className="gain">{activeStats.ready?activeStats.buys:'—'}</b></span><span><small>Sells</small><b className="loss">{activeStats.ready?activeStats.sells:'—'}</b></span><span><small>Change</small><b className={activeStats.change>=0?'gain':'loss'}>{activeStats.ready?pct(activeStats.change):'—'}</b></span></div><div className="panel-body">
      <div className="meme-trade-head"><div><Zap size={14}/><span><b>INSTANT BUY</b><small>One tap · simulated PAPER only</small></span></div><span>{money(cash)} PAPER</span></div>
      ...
      <div className="custom-buy-row"><label><span>CUSTOM BUY</span><div><input aria-label="Custom instant buy SOL" className="amount" type="number" min=".001" max="100" step=".001" value={amount} onChange={e=>{const value=Math.max(.001,Math.min(100,Number(e.target.value)||.001));setAmount(value);localStorage.setItem('paper.quickBuySize',String(value))}}/><b>SOL</b></div></label><button type="button" className="paper-buy custom-paper-buy" disabled={busy||!selected||!canBuy||dataStatus==='STALE'} onClick={()=>void executeTrade(selected,'buy',{amountSol:amount})}>{busy?'WORKING…':'BUY '+(selected?'$'+selected.symbol:'TOKEN')+' · PAPER'}</button></div>
      ...
      <div className="percentage-sell-grid">{[25,50,75,100].map(value=><button key={value} type="button" className="percentage-sell-button" disabled={busy||!selected||!selectedPosition||dataStatus==='STALE'} onClick={()=>{setSellPct(value);void executeTrade(selected,'sell',{sellPct:value})}}><b>{value}%</b><small>{selectedPosition?money(positionValue*value/100):'—'}</small></button>)}</div>
      <section className={'advanced-orders-shell '+(advancedOpen?'open':'')}><button type="button" className="advanced-orders-toggle" aria-expanded={advancedOpen} aria-controls="advanced-orders-panel" onClick={()=>setAdvancedOpen(v=>!v)}>Advanced orders</button>{advancedOpen&&<div id="advanced-orders-panel"><div className="advanced-order-side"><button type="button" className={side==='buy'?'active':''} onClick={()=>setSide('buy')}>Buy order</button><button type="button" className={side==='sell'?'active':''} onClick={()=>setSide('sell')}>Sell order</button></div><AdvancedOrderPanel token={selected} side={side} amountSol={buySol} sellPct={sellPct} hasPosition={Boolean(selectedPosition)} onChanged={()=>window.dispatchEvent(new Event('paper:account-changed'))}/></div>}</section>
      <div className="slippage-control"><label>MAX SLIPPAGE</label><div>{[1,5,15,30].map(v=><button key={v} className={maxSlippagePct===v?'active':''} onClick={()=>setMaxSlippagePct(v)}>{v}%</button>)}<input aria-label="Custom max slippage" type="number" min="0.1" max="50" step="0.1" value={maxSlippagePct} onChange={e=>setMaxSlippagePct(Math.max(.1,Math.min(50,Number(e.target.value)||15)))}/></div><small>PAPER uses a live requote and simulated price impact; excessive slippage is rejected.</small></div>
      <div className="trade-safety-line"><span className={`data-dot ${dataStatus.toLowerCase()}`}/><b>{dataStatus}</b><span>Simulated only · no blockchain transaction</span></div>{dataStatus==='STALE'&&<div className="trade-message loss">Live pricing is stale. New PAPER orders are paused until fresh data returns. <button onClick={()=>void loadFeed(true)}>Retry market data</button></div>}{message&&<div className={`trade-message ${/rejected|error|unavailable|not enough|no open/i.test(message)?'loss':''}`}>{message}</div>}
