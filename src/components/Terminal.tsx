...
type FillReceipt={side:Side;symbol:string;amountUsd:number;referencePrice:number;fillPrice:number;postTradePrice:number;impactPct:number;poolImpactPct:number;effectiveSlippageBps:number;slippageLimitBps:number;feeUsd:number;dexFeeUsd:number;dexFeeBps:number;networkFeeUsd:number;priorityFeeSol:number;quality:string;ageMs:number;orderId?:string;replayed?:boolean}
...
export default function Terminal(){
  ...
  const [selectedUpdatedAt,setSelectedUpdatedAt]=useState(0),[receipt,setReceipt]=useState<FillReceipt|null>(null),[instantMode,setInstantMode]=useState(false),[confirmOrders,setConfirmOrders]=useState(false),[statsWindow,setStatsWindow]=useState<StatsWindow>('5m')
  const [slippageBps,setSlippageBps]=useState(1000),[priorityFeeSol,setPriorityFeeSol]=useState(0),[dexFeeBps,setDexFeeBps]=useState(30)
  ...
  useEffect(()=>{const saved=Number(localStorage.getItem('paper.quickBuySize')),savedMode=localStorage.getItem('paper.tradeAmountMode');if(saved>0)setAmount(saved);if(savedMode==='usd'||savedMode==='sol')setAmountMode(savedMode);setInstantMode(localStorage.getItem('paper.instantMode')==='1');setConfirmOrders(localStorage.getItem('paper.confirmOrders')==='1');try{const exec=JSON.parse(localStorage.getItem('paper.execution.settings.v1')||'{}');if(Number(exec.slippageBps)>=10)setSlippageBps(Math.min(5000,Number(exec.slippageBps)));if(Number(exec.priorityFeeSol)>=0)setPriorityFeeSol(Math.min(.1,Number(exec.priorityFeeSol)));if(Number(exec.dexFeeBps)>=0)setDexFeeBps(Math.min(500,Number(exec.dexFeeBps)))}catch{}},[])
  ...
  useEffect(()=>{localStorage.setItem('paper.execution.settings.v1',JSON.stringify({slippageBps,priorityFeeSol,dexFeeBps}))},[slippageBps,priorityFeeSol,dexFeeBps])
  ...
  const cash=Number(account?.cash_usd||0),nativeEquivalent=solUsd>0?cash/solUsd:0,buySol=amountMode==='sol'?amount:(solUsd>0?amount/solUsd:0),buyUsd=amountMode==='usd'?amount:amount*solUsd,estimatedFee=buyUsd*.01,estimatedNetworkFeeUsd=solUsd*(.000005+priorityFeeSol),estimatedDexFeeUsd=buyUsd*(dexFeeBps/10000),totalDebit=buyUsd+estimatedFee+estimatedNetworkFeeUsd
  ...
  async function executeTrade(target=selected,action=side){
    ...
    try{
      const idempotencyKey=crypto.randomUUID(),execution={slippageBps,priorityFeeSol,dexFeeBps},body=action==='buy'?{mint:target.mint,side:'buy',amountSol:buySol,idempotencyKey,...execution}:{mint:target.mint,side:'sell',sellPct,idempotencyKey,...execution}
      ...
      setReceipt({side:action,symbol:target.symbol,amountUsd:Number(fill.requestedAmountUsd||result.gross_usd||0),referencePrice:Number(market.referencePriceUsd||0),fillPrice:Number(fill.simulatedFillPriceUsd||0),postTradePrice:Number(fill.postTradePriceUsd||0),impactPct:Number(fill.priceImpactPct||0),poolImpactPct:Number(fill.poolImpactPct||0),effectiveSlippageBps:Number(fill.effectiveSlippageBps||0),slippageLimitBps:Number(fill.slippageLimitBps||slippageBps),feeUsd:Number(fill.paperFeeUsd||0),dexFeeUsd:Number(fill.dexFeeUsd||0),dexFeeBps:Number(fill.dexFeeBps||dexFeeBps),networkFeeUsd:Number(fill.networkFeeUsd||0),priorityFeeSol:Number(fill.priorityFeeSol||priorityFeeSol),quality:String(fill.executionQuality||'modeled'),ageMs:Number(market.marketDataAgeMs||0),orderId:String(result.order_id||''),replayed:Boolean(result.replayed)})
  ...
  return <div className="ax-app"><AppHeader active="spot"/><nav className="mobile-terminal-tabs"><button className={mobilePane==='chart'?'active':''} onClick={()=>setMobilePane('chart')}>Chart</button><button className={mobilePane==='trade'?'active':''} onClick={()=>setMobilePane('trade')}>Trade</button><button className={mobilePane==='positions'?'active':''} onClick={()=>setMobilePane('positions')}>Positions</button><button className={mobilePane==='info'?'active':''} onClick={()=>setMobilePane('info')}>Info</button></nav><main data-mobile-pane={mobilePane} className="spot-detail phase1-terminal parts23-terminal" style={{'--paper-token-rail':tokenRailWidth+'px','--paper-trade-panel':tradePanelWidth+'px'} as CSSProperties}>
    ...
    <button className="paper-panel-resizer trade-resizer" aria-label="Resize trade panel" onPointerDown={e=>beginPanelResize('trade',e)}/><aside className="spot-trade-panel axiom-paper-trade p23-trade-panel"><div className="trade-window-tabs">{(['1m','5m','1h','6h','24h'] as StatsWindow[]).map(w=><button key={w} className={statsWindow===w?'active':''} onClick={()=>setStatsWindow(w)}>{w}</button>)}</div><div className="trade-stats-top"><span><small>{activeStats.label} Vol</small><b>{activeStats.ready?money(activeStats.volume):'warming'}</b></span><span><small>Buys</small><b className="gain">{activeStats.ready?activeStats.buys:'—'}</b></span><span><small>Sells</small><b className="loss">{activeStats.ready?activeStats.sells:'—'}</b></span><span><small>Change</small><b className={activeStats.change>=0?'gain':'loss'}>{activeStats.ready?pct(activeStats.change):'—'}</b></span></div><div className="panel-body"><div className="side-switch"><button className={side==='buy'?'buy':''} onClick={()=>setSide('buy')}>Buy</button><button className={side==='sell'?'sell':''} onClick={()=>setSide('sell')}>Sell</button></div><div className="trade-mode-row"><div className="market-mode"><Activity size={12}/><b>Market</b></div><button className={`instant-toggle ${instantMode?'on':''}`} onClick={toggleInstant}><Zap size={12}/> Instant {instantMode?'ON':'OFF'}</button><button className={`confirm-toggle ${confirmOrders?'on':''}`} onClick={toggleConfirm}>Confirm {confirmOrders?'ON':'OFF'}</button><span>{money(cash)} PAPER</span></div><AdvancedOrderPanel token={selected} side={side} amountSol={buySol} sellPct={sellPct} hasPosition={Boolean(selectedPosition)} slippageBps={slippageBps} priorityFeeSol={priorityFeeSol} dexFeeBps={dexFeeBps} onChanged={()=>window.dispatchEvent(new Event('paper:account-changed'))}/>
      {side==='buy'?<><div className="trade-label-row"><label>AMOUNT</label><span>{solUsd?`${money(buyUsd)} PAPER`:'SOL price loading…'}</span></div><div className="amount-with-symbol p23-amount"><input className="amount" type="number" min={amountMode==='sol'?'.001':'1'} step={amountMode==='sol'?'.001':'1'} value={amount} onChange={e=>setAmount(Math.max(amountMode==='sol'?.001:1,Number(e.target.value)||0))}/><div className="amount-mode"><button className={amountMode==='sol'?'active':''} onClick={()=>setAmountMode('sol')}>SOL</button><button className={amountMode==='usd'?'active':''} onClick={()=>setAmountMode('usd')}>$</button></div></div><div className="preset-grid">{presets.map(v=><button key={v} className={`preset ${amount===v?'selected':''}`} onClick={()=>setAmount(v)}>{amountMode==='usd'?`$${v}`:v}</button>)}</div><div className="trade-estimate-card"><div><span>PAPER buying power</span><b>{money(cash)}</b></div><div><span>Order notional</span><b>{money(buyUsd)}</b></div><div><span>Est. PAPER fee</span><b>{money(estimatedFee)}</b></div><div><span>Est. network fee</span><b>{money(estimatedNetworkFeeUsd)}</b></div><div><span>Assumed DEX fee</span><b>{money(estimatedDexFeeUsd)}</b></div><div><span>Est. total debit</span><b>{money(totalDebit)}</b></div></div></>:<><div className="trade-label-row"><label>SELL POSITION</label><span>{selectedPosition?`${Number(selectedPosition.quantity_tokens).toLocaleString()} tokens`:'No position'}</span></div><div className="preset-grid sell-presets">{[25,50,75,100].map(v=><button key={v} className={`preset ${sellPct===v?'selected':''}`} onClick={()=>setSellPct(v)}>{v}%</button>)}</div>{selectedPosition&&<div className="trade-estimate-card"><div><span>Cost basis</span><b>{money(Number(selectedPosition.cost_basis_usd||0))}</b></div><div><span>Entry price</span><b>{money(Number(selectedPosition.average_entry_price_usd))}</b></div><div><span>Entry MC</span><b>{money(Number(selectedPosition.average_entry_mc_usd||0))}</b></div><div><span>Sell size</span><b>{sellPct}%</b></div></div>}</>}
      <div className="execution-settings"><label>Slippage limit<select aria-label="Slippage limit" value={slippageBps} onChange={e=>setSlippageBps(Number(e.target.value))}><option value={100}>1%</option><option value={300}>3%</option><option value={500}>5%</option><option value={1000}>10%</option><option value={2000}>20%</option><option value={5000}>50%</option></select></label><label>Priority fee (SOL)<select aria-label="Priority fee" value={priorityFeeSol} onChange={e=>setPriorityFeeSol(Number(e.target.value))}><option value={0}>0</option><option value={0.0001}>0.0001</option><option value={0.0005}>0.0005</option><option value={0.001}>0.001</option><option value={0.005}>0.005</option></select></label><label>Assumed DEX fee<select aria-label="Assumed DEX fee" value={dexFeeBps} onChange={e=>setDexFeeBps(Number(e.target.value))}><option value={20}>0.20%</option><option value={30}>0.30%</option><option value={50}>0.50%</option><option value={100}>1.00%</option></select></label></div><div className="execution-model-note"><span><small>EXECUTION MODEL</small><b>Constant-product</b></span><span><small>BASE NETWORK FEE</small><b>0.000005 SOL</b></span><span><small>DEX FEE</small><b>Simulation assumption</b></span></div><button className={`paper-buy p23-primary-trade ${side==='sell'?'paper-sell':''}`} disabled={busy||!selected||(side==='buy'&&!canBuy)||(side==='sell'&&!selectedPosition)||dataStatus==='STALE'} onClick={()=>void executeTrade()}>{busy?'CHECKING LIVE MARKET…':`${instantMode?'INSTANT ':''}${side==='buy'?'BUY':'SELL'} ${selected?`$${selected.symbol}`:'TOKEN'} · PAPER`}</button><div className="trade-safety-line"><span className={`data-dot ${dataStatus.toLowerCase()}`}/><b>{dataStatus}</b><span>Simulated only · no blockchain transaction</span></div>{dataStatus==='STALE'&&<div className="trade-message loss">Live pricing is stale. New PAPER orders are paused until fresh data returns.</div>}{message&&<div className={`trade-message ${/rejected|error|unavailable|not enough|no open/i.test(message)?'loss':''}`}>{message}</div>}
      {receipt&&<div className="fill-receipt p23-receipt"><div className="fill-receipt-head"><b>{receipt.replayed?'PAPER ORDER REPLAYED':`PAPER ${receipt.side.toUpperCase()} FILLED`}</b><span>{receipt.quality.toUpperCase()}</span></div><dl><div><dt>Notional</dt><dd>{money(receipt.amountUsd)}</dd></div><div><dt>Reference</dt><dd>{money(receipt.referencePrice)}</dd></div><div><dt>Simulated fill</dt><dd>{money(receipt.fillPrice)}</dd></div><div><dt>Post-trade price</dt><dd>{money(receipt.postTradePrice)}</dd></div><div><dt>Pool impact</dt><dd>{receipt.poolImpactPct.toFixed(3)}%</dd></div><div><dt>Effective slippage</dt><dd>{(receipt.effectiveSlippageBps/100).toFixed(3)}% / {(receipt.slippageLimitBps/100).toFixed(1)}% max</dd></div><div><dt>PAPER fee</dt><dd>{money(receipt.feeUsd)}</dd></div><div><dt>DEX fee assumption</dt><dd>{money(receipt.dexFeeUsd)} · {(receipt.dexFeeBps/100).toFixed(2)}%</dd></div><div><dt>Network fee</dt><dd>{money(receipt.networkFeeUsd)} · priority {receipt.priorityFeeSol.toFixed(6)} SOL</dd></div><div><dt>Data age</dt><dd>{(receipt.ageMs/1000).toFixed(2)}s</dd></div></dl>{receipt.orderId&&<div className="receipt-order">Order {short(receipt.orderId)}</div>}</div>}
