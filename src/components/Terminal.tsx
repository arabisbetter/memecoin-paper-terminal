...
export default function Terminal(){
  ...
  const loadAccount=useCallback(async(id?:string)=>{
    ...
    const live=await Promise.all(base.map(async row=>{
      ...
      if(!mint)return row
      try{const r=await fetch(`/api/market/token/${mint}`,{cache:'no-store'});const j=await r.json();return r.ok?{...row,current:j.token as MarketToken}:row}catch{return row}
    }))
    setPositions(live)
  },[supabase])
  ...
  useEffect(()=>{
    if(!selected)return
    const id=setInterval(async()=>{try{const r=await fetch(`/api/market/token/${selected.mint}`,{cache:'no-store'});const j=await r.json();if(r.ok&&j.token)setSelected(j.token)}catch{}},3000)
