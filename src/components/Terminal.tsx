...
type FillReceipt={side:Side;symbol:string;amountUsd:number;referencePrice:number;fillPrice:number;impactPct:number;feeUsd:number;quality:string;ageMs:number;orderId?:string;replayed?:boolean}
...
export default function Terminal(){
  ...
  const [selectedUpdatedAt,setSelectedUpdatedAt]=useState(0),[receipt,setReceipt]=useState<FillReceipt|null>(null),[instantMode,setInstantMode]=useState(false),[statsWindow,setStatsWindow]=useState<StatsWindow>('5m')
  const [feedSource,setFeedSource]=useState(''),[feedAsOf,setFeedAsOf]=useState(0)
  const feedBusy=useRef(false),accountBusy=useRef(false)

  const chooseToken=useCallback((token:MarketToken,syncUrl=true)=>{
    setSelected(token);setSelectedUpdatedAt(Date.now());setReceipt(null);setMessage('')
  ...
  const loadAccount=useCallback(async(id?:string,force=false)=>{
    ...
    try{
      ...
      const sj=await solRes.json().catch(()=>null);if(solRes.ok)setSolUsd(Number(sj?.priceUsd||0))
  ...
  const cash=Number(account?.cash_usd||0),nativeEquivalent=solUsd>0?cash/solUsd:0,buySol=amountMode==='sol'?amount:(solUsd>0?amount/solUsd:0),buyUsd=amountMode==='usd'?amount:amount*solUsd,estimatedFee=buyUsd*.005,totalDebit=buyUsd+estimatedFee
  const activeStats=statsFor(selected,statsWindow),txCount=activeStats.buys+activeStats.sells,buyPressure=txCount?activeStats.buys/txCount*100:0
  const liquidityRatio=selected?.marketCap?selected.liquidityUsd/selected.marketCap*100:0
  ...
  const presets=amountMode==='sol'?[.01,.1,.5,1]:[10,25,50,100]
  const canBuy=Boolean(selected&&buySol>0&&solUsd>0&&totalDebit<=cash&&dataStatus!=='STALE')

  async function searchToken(e?:FormEvent){
    e?.preventDefault();const q=query.trim();if(!q)return;setLookupBusy(true);setMessage('')
    try{const local=tokens.find(t=>t.mint===q||t.symbol.toLowerCase()===q.toLowerCase()||t.name.toLowerCase()===q.toLowerCase());if(local){chooseToken(local);return}if(!isMint(q)){setMessage('No exact live match. Paste a Solana contract address for direct lookup.');return}const r=await fetch(`/api/market/token/${encodeURIComponent(q)}`,{cache:'no-store'}),j=await r.json();if(!r.ok||!j.token)throw new Error(j.error||'Token found, but no supported active market was detected.');chooseToken(j.token)}catch(e2){setMessage(e2 instanceof Error?e2.message:'Token lookup failed')}finally{setLookupBusy(false)}
  }
  ...
  async function executeTrade(target=selected,action=side){
    ...
    try{
      ...
      setReceipt({side:action,symbol:target.symbol,amountUsd:Number(fill.requestedAmountUsd||result.gross_usd||0),referencePrice:Number(market.referencePriceUsd||0),fillPrice:Number(fill.simulatedFillPriceUsd||0),impactPct:Number(fill.priceImpactPct||0),feeUsd:Number(fill.paperFeeUsd||0),quality:String(fill.executionQuality||'estimated'),ageMs:Number(market.marketDataAgeMs||0),orderId:String(result.order_id||''),replayed:Boolean(result.replayed)})
      setMessage(`${action==='buy'?'PAPER BUY':'PAPER SELL'} FILLED`);chooseToken(target);await loadAccount(userId,true);window.dispatchEvent(new Event('paper:account-changed'))
  ...
  return <div className="ax-app"><AppHeader active="spot"/><main className="spot-detail phase1-terminal parts23-terminal">
    <aside className="spot-token-list p23-token-rail">
      <form className="spot-search p23-search" onSubmit={searchToken}><Search size={13}/><input placeholder="Search ticker, name or CA" value={query} onChange={e=>setQuery(e.target.value)}/><button type="submit" disabled={lookupBusy}>{lookupBusy?'…':'Go'}</button></form>
      <div className="spot-list-head"><span>LIVE MEMECOINS</span><span>{visible.length}</span></div>
      <div className="token-list">{visible.map(t=><div key={t.mint} role="button" tabIndex={0} className={`token-row-shell ${selected?.mint===t.mint?'selected':''}`} onClick={()=>chooseToken(t)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();chooseToken(t)}}}><div className="token-avatar">{t.image?<img src={t.image} alt="" loading="lazy"/>:t.symbol.slice(0,2)}</div><div className="token-copy"><div className="token-line"><b>${t.symbol}</b><span>{t.name}</span></div><div className="token-sub"><span>MC {money(t.marketCap)}</span><span>LQ {money(t.liquidityUsd)}</span></div></div><div className="token-price"><b>{money(t.priceUsd)}</b><span className={t.priceChange5m>=0?'gain':'loss'}>{pct(Number(t.priceChange5m||0))}</span></div>{instantMode&&<button className="row-instant-buy" title={`Instant PAPER buy ${amountMode==='sol'?`${amount} SOL`:money(amount)}`} disabled={busy||!solUsd} onClick={e=>{e.stopPropagation();void instantBuy(t)}}><Zap size={12}/></button>}</div>)}</div>
      <div className="rail-feed-status"><i/><span>{feedSource||'market feed'}</span><small>{feedAsOf?new Date(feedAsOf).toLocaleTimeString():''}</small></div>
    </aside>

    <section className="spot-market p23-market">
      <div className="token-head p23-token-head">{selected?<><div className="token-avatar large">{selected.image?<img src={selected.image} alt=""/>:selected.symbol.slice(0,2)}</div><div className="p23-token-identity"><div className="token-title">${selected.symbol}<span>{selected.name}</span></div><div className="token-link-row"><button className="mint-button" onClick={()=>void copyMint()}><Copy size={10}/>{copied?'Copied':short(selected.mint)}</button><span className="venue-pill">{selected.dexId||'Solana'}</span>{selected.website&&<a href={selected.website} target="_blank" rel="noreferrer"><Globe2 size={11}/> Web</a>}{selected.twitter&&<a href={selected.twitter} target="_blank" rel="noreferrer">X</a>}{selected.telegram&&<a href={selected.telegram} target="_blank" rel="noreferrer"><MessageCircle size={11}/> TG</a>}</div></div><div className="spacer"/><div className="headline-stat"><small>VIEWS</small><b className="view-stat"><Eye size={12}/>{viewCount.toLocaleString()}</b></div><div className="headline-stat"><small>PRICE</small><b>{money(selected.priceUsd)}</b></div><div className="headline-stat"><small>5M</small><b className={Number(selected.priceChange5m||0)>=0?'gain':'loss'}>{pct(Number(selected.priceChange5m||0))}</b></div><div className="headline-stat"><small>MC</small><b>{money(selected.marketCap)}</b></div><span className={`data-health ${dataStatus.toLowerCase()}`}>{dataStatus}</span></>:<span>Select a live memecoin</span>}</div>

      <CandleChart poolAddress={selected?.pairAddress} currentPrice={selected?.priceUsd} symbol={selected?.symbol}/>

      <div className="metric-strip p23-metric-strip">{[['Market Cap',selected?money(selected.marketCap):'—'],['Liquidity',selected?money(selected.liquidityUsd):'—'],['5m Volume',selected?money(Number(selected.volume5m||0)):'—'],['1h Volume',selected?money(Number(selected.volume1h||0)):'—'],['24h Volume',selected?money(Number(selected.volume24h||0)):'—'],['Age',selected?age(selected.pairCreatedAt):'—']].map(([l,v])=><div className="metric" key={l}><small>{l}</small><b>{v}</b></div>)}</div>
    ...
    <aside className="spot-trade-panel axiom-paper-trade p23-trade-panel">
      <div className="trade-window-tabs">{(['1m','5m','1h','6h','24h'] as StatsWindow[]).map(w=><button key={w} className={statsWindow===w?'active':''} onClick={()=>setStatsWindow(w)}>{w}</button>)}</div>
      <div className="trade-stats-top"><span><small>{activeStats.label} Vol</small><b>{activeStats.ready?money(activeStats.volume):'warming'}</b></span><span><small>Buys</small><b className="gain">{activeStats.ready?activeStats.buys:'—'}</b></span><span><small>Sells</small><b className="loss">{activeStats.ready?activeStats.sells:'—'}</b></span><span><small>Change</small><b className={activeStats.change>=0?'gain':'loss'}>{activeStats.ready?pct(activeStats.change):'—'}</b></span></div>
      <div className="panel-body">
        ...
        <div className="trade-mode-row"><div className="market-mode"><Activity size={12}/><b>Market</b></div><button className={`instant-toggle ${instantMode?'on':''}`} onClick={toggleInstant}><Zap size={12}/> Instant {instantMode?'ON':'OFF'}</button><span>{money(cash)} PAPER</span></div>

        {side==='buy'?<><div className="trade-label-row"><label>AMOUNT</label><span>{solUsd?`${money(buyUsd)} PAPER`:'SOL price loading…'}</span></div><div className="amount-with-symbol p23-amount"><input className="amount" type="number" min={amountMode==='sol'?'.001':'1'} step={amountMode==='sol'?'.001':'1'} value={amount} onChange={e=>setAmount(Math.max(amountMode==='sol'?.001:1,Number(e.target.value)||0))}/><div className="amount-mode"><button className={amountMode==='sol'?'active':''} onClick={()=>setAmountMode('sol')}>SOL</button><button className={amountMode==='usd'?'active':''} onClick={()=>setAmountMode('usd')}>$</button></div></div><div className="preset-grid">{presets.map(v=><button key={v} className={`preset ${amount===v?'selected':''}`} onClick={()=>setAmount(v)}>{amountMode==='usd'?`$${v}`:v}</button>)}</div><div className="trade-estimate-card"><div><span>PAPER buying power</span><b>{money(cash)}</b></div><div><span>Order notional</span><b>{money(buyUsd)}</b></div><div><span>Est. PAPER fee</span><b>{money(estimatedFee)}</b></div><div><span>Est. total debit</span><b>{money(totalDebit)}</b></div></div></>:<><div className="trade-label-row"><label>SELL POSITION</label><span>{selectedPosition?`${Number(selectedPosition.quantity_tokens).toLocaleString()} tokens`:'No position'}</span></div><div className="preset-grid sell-presets">{[10,25,50,100].map(v=><button key={v} className={`preset ${sellPct===v?'selected':''}`} onClick={()=>setSellPct(v)}>{v}%</button>)}</div>{selectedPosition&&<div className="trade-estimate-card"><div><span>Cost basis</span><b>{money(Number(selectedPosition.cost_basis_usd||0))}</b></div><div><span>Entry price</span><b>{money(Number(selectedPosition.average_entry_price_usd))}</b></div><div><span>Entry MC</span><b>{money(Number(selectedPosition.average_entry_mc_usd||0))}</b></div><div><span>Sell size</span><b>{sellPct}%</b></div></div>}</>}

        <button className={`paper-buy p23-primary-trade ${side==='sell'?'paper-sell':''}`} disabled={busy||!selected||(side==='buy'&&!canBuy)||(side==='sell'&&!selectedPosition)||dataStatus==='STALE'} onClick={()=>void executeTrade()}>{busy?'CHECKING LIVE MARKET…':`${instantMode?'INSTANT ':''}${side==='buy'?'BUY':'SELL'} ${selected?`$${selected.symbol}`:'TOKEN'} · PAPER`}</button>
        <div className="trade-safety-line"><span className={`data-dot ${dataStatus.toLowerCase()}`}/><b>{dataStatus}</b><span>Simulated only · no blockchain transaction</span></div>
        {dataStatus==='STALE'&&<div className="trade-message loss">Live pricing is stale. New PAPER orders are paused until fresh data returns.</div>}{message&&<div className={`trade-message ${/rejected|error|unavailable|not enough|no open/i.test(message)?'loss':''}`}>{message}</div>}

        {receipt&&<div className="fill-receipt p23-receipt"><div className="fill-receipt-head"><b>{receipt.replayed?'PAPER ORDER REPLAYED':`PAPER ${receipt.side.toUpperCase()} FILLED`}</b><span>{receipt.quality.toUpperCase()}</span></div><dl><div><dt>Notional</dt><dd>{money(receipt.amountUsd)}</dd></div><div><dt>Reference</dt><dd>{money(receipt.referencePrice)}</dd></div><div><dt>Simulated fill</dt><dd>{money(receipt.fillPrice)}</dd></div><div><dt>Impact</dt><dd>{receipt.impactPct.toFixed(3)}%</dd></div><div><dt>PAPER fee</dt><dd>{money(receipt.feeUsd)}</dd></div><div><dt>Data age</dt><dd>{(receipt.ageMs/1000).toFixed(2)}s</dd></div></dl>{receipt.orderId&&<div className="receipt-order">Order {short(receipt.orderId)}</div>}</div>}

        <div className="pressure-meter"><div className="pressure-label"><span>Buy pressure · {activeStats.label}</span><b>{activeStats.ready?`${buyPressure.toFixed(0)}%`:'warming'}</b></div><div className="pressure-track"><i style={{width:`${activeStats.ready?buyPressure:50}%`}}/></div></div>
        ...
        {(selected?.buyUrl||selected?.pairUrl)&&<a className="trade-real-link" href={selected.buyUrl||selected.pairUrl} target="_blank" rel="noreferrer">Open real market <ExternalLink size={11}/></a>}
      </div>
    </aside>
