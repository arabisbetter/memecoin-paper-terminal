...
export default function AppHeader({active}:{active:Active}){
  ...
  async function readAll(userId:string){
    ...
    if(ae)throw ae
    if(le)throw le
    const accepted=new Set((l||[]).map(r=>`${r.document_type}:${r.document_version}`))
  ...
  useEffect(()=>{
    if(!uid)return
    const refresh=()=>void readAll(uid)
    window.addEventListener('paper:account-changed',refresh)
    const id=window.setInterval(()=>{if(!document.hidden)refresh()},7000)
    return()=>{window.clearInterval(id);window.removeEventListener('paper:account-changed',refresh)}
  },[uid])
  ...
  async function finishOnboarding(){
    ...
    try{
      ...
      const {data,error}=await supabase.from('profiles').update({username:u,display_name:d,avatar_url:avatarUrl,profile_completed:true,updated_at:new Date().toISOString()}).eq('id',uid).select('id,username,display_name,avatar_url,avatar_emoji,profile_completed').single()
      if(error)throw error
  ...
  async function acceptLegal(){
    ...
    try{
      const {data,error}=await supabase.from('legal_acceptances').select('document_type,document_version').eq('user_id',uid)
      if(error)throw error
      const existing=new Set((data||[]).map(r=>`${r.document_type}:${r.document_version}`))
      const rows=requiredLegal.filter(([type,version])=>!existing.has(`${type}:${version}`)).map(([document_type,document_version])=>({user_id:uid,document_type,document_version}))
      if(rows.length){const {error:insertError}=await supabase.from('legal_acceptances').insert(rows);if(insertError)throw insertError}
      setLegalAccepted(true)
    }catch(e){setMessage(e instanceof Error?e.message:'Could not record acceptance')}
    finally{setLegalSaving(false)}
  }

  const pfp=profile?.avatar_url||avatarPreview
  const spotActive=active==='discover'||active==='spot'
  const accountError=!starting&&(!profile||!account)&&Boolean(message)
  const cash=Number(account?.cash_usd||0)
  const solEquivalent=solUsd>0?cash/solUsd:0
  const balanceText=starting?'Starting…':account?`$${cash.toFixed(2)} PAPER`:'—'
  ...
  return <>
    <header className="ax-header">
      ...
      <Link className="icon-control" href="/leaderboards" title="Leaderboard"><BarChart3 size={15}/></Link>
      ...
      <div className={`ax-balance-pill ${accountError?'account-error-pill':''}`} title={accountError?message:solEquivalent?`${balanceText} · ≈ ${solEquivalent.toFixed(3)} SOL at current price`:'PAPER buying power'}><span className="paper-cash-mark">$</span>{accountError?'ACCOUNT ERROR':balanceText}</div>
