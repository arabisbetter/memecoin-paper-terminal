...
export default function AppHeader({active}:{active:Active}){
  ...
  async function acceptLegal(){
    ...
    try{const {data:{session}}=await supabase.auth.getSession();if(!session?.access_token)throw new Error('Session unavailable');const r=await fetch('/api/legal/accept',{method:'POST',headers:{Authorization:`Bearer ${session.access_token}`}});const body=await r.json().catch(()=>({}));if(!r.ok)throw new Error(body.error||'Could not record acceptance');setLegalAccepted(true)}catch(e){setMessage(e instanceof Error?e.message:'Could not record acceptance')}finally{setLegalSaving(false)}
  }

  const pfp=profile?.avatar_url||avatarPreview,accountError=!starting&&(!profile||!account)&&Boolean(message),cash=Number(account?.cash_usd||0),solEquivalent=solUsd>0?cash/solUsd:0,balanceText=starting?'Starting…':account?`${cash.toFixed(2)} PAPER SOL`:'—'

  return <>
    <header className="ax-header final-header master-header"><Link href="/" className="ax-logo"><span className="ax-mark">P</span><span>APER</span></Link><nav className="master-nav" aria-label="Primary navigation"><Link className={currentActive==='discover'?'active':''} href="/discover">Discover</Link><Link className={currentActive==='spot'?'active':''} href="/spot">Spot</Link><Link className={currentActive==='pulse'?'active':''} href="/pulse">Pulse</Link><Link className={currentActive==='evaluation'?'active':''} href="/evaluation">Evaluation</Link><Link className={currentActive==='chains'?'active':''} href="/chains">Chains</Link><Link className={currentActive==='portfolio'?'active':''} href="/portfolio">Portfolio</Link><Link className={currentActive==='watchlist'?'active':''} href="/watchlist">Watchlist</Link><Link className={currentActive==='leaderboard'?'active':''} href="/leaderboards">Leaderboard</Link><FeedbackButton/></nav><HeaderSearch/><div className="ax-header-spacer"/><div className="header-viewer-pill" title={`Unique PAPER viewers active in the last ${viewerStats.windowMinutes} minutes`}><Eye size={12}/><b>{viewerStats.ready?viewerStats.total.toLocaleString():'—'}</b><span>viewers</span></div><div className={'ax-balance-pill '+(accountError?'account-error-pill':'')} title={accountError?message:solEquivalent?balanceText+' · ≈ '+solEquivalent.toFixed(3)+' SOL at current price':'PAPER buying power'}><span className="paper-cash-mark">$</span>{accountError?'ACCOUNT ERROR':balanceText}</div><Link href="/profile" className={'ax-profile-entry '+(currentActive==='profile'?'active':'')} aria-label="Profile">{pfp?<img src={pfp} alt="Profile"/>:<span className="profile-fallback">{profile?.avatar_emoji||'◢'}</span>}<span className="profile-entry-copy"><b>{profile?.username||'Profile'}</b><small>Profile</small></span></Link></header>
