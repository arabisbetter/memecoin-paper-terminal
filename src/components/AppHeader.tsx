...
export default function AppHeader({active}:{active:Active}){
  ...
  useEffect(()=>{
    ...
    const refresh=async()=>{
      ...
      if(alive&&data){const p=data as Profile;setProfile(p);if(p.avatar_url)setAvatarPreview(p.avatar_url)}
    }
    const onChanged=()=>void refresh()
    window.addEventListener('paper:account-changed',onChanged)
    const id=setInterval(refresh,10000)
    return()=>{alive=false;clearInterval(id);window.removeEventListener('paper:account-changed',onChanged)}
  },[supabase,uid])
  ...
  const pfp=profile?.avatar_url||avatarPreview;const spotActive=active==='discover'||active==='spot'
  return <>
    <header className="ax-header"><Link href="/" className="ax-logo"><span className="ax-mark">▲</span><span>PAPER</span></Link><nav className="ax-nav ax-nav-minimal"><Link className={spotActive?'active':''} href="/">Spot</Link><Link className={active==='pulse'?'active':''} href="/pulse">Pulse</Link></nav><div className="ax-header-spacer"/><Link className="icon-control" href="/leaderboards" title="PAPER PnL leaderboard"><BarChart3 size={15}/></Link><Link className="icon-control" href="/pulse" title="Live Pulse"><Radio size={15}/></Link><Link className="icon-control" href="/profile" title="Settings"><Settings2 size={15}/></Link><div className="ax-balance-pill"><span className="sol-dot">≋</span>{profile?Number(profile.paper_cash_sol).toFixed(2):(starting?'Starting…':'Error')} <small>PAPER SOL</small></div><Link href="/profile" className={`ax-pfp ${active==='profile'?'active':''}`}>{pfp?<img src={pfp} alt="Profile"/>:<span>{profile?.avatar_emoji||'◢'}</span>}</Link></header>
