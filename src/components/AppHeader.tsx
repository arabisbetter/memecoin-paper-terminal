...
type Active='discover'|'pulse'|'spot'|'evaluation'|'portfolio'|'leaderboard'|'profile'|'wallets'|'rewards'|'chains'|'watchlist'

export default function AppHeader({active}:{active:Active}){
  const pathname=usePathname()
  const currentActive:Active=pathname.startsWith('/discover')?'discover':pathname.startsWith('/spot')?'spot':pathname.startsWith('/evaluation')?'evaluation':pathname.startsWith('/pulse')?'pulse':pathname.startsWith('/portfolio')?'portfolio':pathname.startsWith('/leaderboards')?'leaderboard':pathname.startsWith('/profile')?'profile':pathname.startsWith('/wallets')?'wallets':pathname.startsWith('/rewards')?'rewards':pathname.startsWith('/chains')?'chains':pathname.startsWith('/watchlist')?'watchlist':active
  ...
  return <>
    <header className="ax-header final-header"><Link href="/" className="ax-logo"><span className="ax-mark">P</span><span>PAPER</span></Link><nav className="ax-nav ax-nav-minimal"><Link className={currentActive==='discover'?'active':''} href="/discover">Discover</Link><Link className={currentActive==='spot'?'active':''} href="/spot">Trade</Link><Link className={currentActive==='evaluation'?'active':''} href="/evaluation">Evaluation</Link><Link className={currentActive==='pulse'?'active':''} href="/pulse">Pulse</Link><Link className={currentActive==='chains'?'active':''} href="/chains">Chains</Link><Link className={currentActive==='portfolio'?'active':''} href="/portfolio">Portfolio</Link></nav><div className="ax-header-spacer"/><Link className={currentActive==='wallets'?'icon-control active':'icon-control'} href="/wallets" title="Wallet tracker"><WalletCards size={15}/></Link><Link className={currentActive==='watchlist'?'icon-control active':'icon-control'} href="/watchlist" title="Watchlist"><Star size={15}/></Link><Link className={currentActive==='rewards'?'icon-control active':'icon-control'} href="/rewards" title="Rewards"><Gift size={15}/></Link><Link className={currentActive==='leaderboard'?'icon-control active':'icon-control'} href="/leaderboards" title="Leaderboard"><BarChart3 size={15}/></Link><Link className="icon-control" href="/chains" title="Multi-chain"><Layers3 size={15}/></Link><Link className="icon-control" href="/portfolio" title="PAPER portfolio"><BriefcaseBusiness size={15}/></Link><Link className="icon-control" href="/profile" title="Settings"><Settings2 size={15}/></Link><div className={`ax-balance-pill ${accountError?'account-error-pill':''}`} title={accountError?message:solEquivalent?`${balanceText} · ≈ ${solEquivalent.toFixed(3)} SOL at current price`:'PAPER buying power'}><span className="paper-cash-mark">$</span>{accountError?'ACCOUNT ERROR':balanceText}</div><Link href="/profile" className={`ax-pfp ${currentActive==='profile'?'active':''}`}>{pfp?<img src={pfp} alt="Profile"/>:<span>{profile?.avatar_emoji||'◢'}</span>}</Link></header>
