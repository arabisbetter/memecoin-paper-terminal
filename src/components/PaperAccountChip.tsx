...
export default function PaperAccountChip(){
  ...
  const load=useCallback(async()=>{
    if(!supabase)return
    try{const {data:{user}}=await supabase.auth.getUser();if(!user)return;const [{data:a},solRes]=await Promise.all([supabase.from('paper_accounts').select('cash_usd,starting_balance_usd').eq('user_id',user.id).maybeSingle(),fetch('/api/market/sol',{cache:'no-store'}).catch(()=>null)]);if(a)setAccount(a as Account);if(solRes?.ok){const j=await solRes.json().catch(()=>null);setSolUsd(Number(j?.priceUsd||0))}}finally{setLoading(false)}
  },[supabase])
