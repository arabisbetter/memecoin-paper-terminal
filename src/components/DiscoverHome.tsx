...
const changeFor=(t:MarketToken,tf:Timeframe)=>Number(tf==='5m'?t.priceChange5m:tf==='1h'?t.priceChange1h:tf==='6h'?t.priceChange6h:t.priceChange24h)||0
const volumeFor=(t:MarketToken,tf:Timeframe)=>Number(tf==='5m'?t.volume5m:tf==='1h'?t.volume1h:tf==='6h'?t.volume6h:t.volume24h)||0
const buysFor=(t:MarketToken,tf:Timeframe)=>Number(tf==='5m'?t.buys5m:tf==='1h'?t.buys1h:tf==='6h'?t.buys6h:t.buys24h)||0
const sellsFor=(t:MarketToken,tf:Timeframe)=>Number(tf==='5m'?t.sells5m:tf==='1h'?t.sells1h:tf==='6h'?t.sells6h:t.sells24h)||0
...
export default function DiscoverHome(){
  ...
  async function loadViews(next:MarketToken[]){
    if(!supabase||!next.length)return
  ...
  useEffect(()=>{void load(true);const id=window.setInterval(()=>void load(),12_000);const onVisible=()=>{if(!document.hidden)void load(true)};document.addEventListener('visibilitychange',onVisible);return()=>{window.clearInterval(id);document.removeEventListener('visibilitychange',onVisible)}},[])
  ...
  return <div className="ax-app">
    ...
    <main className="market-console">
      <section className="market-topbar">
        ...
        <div className="time-tabs-terminal">{(['5m','1h','6h','24h'] as Timeframe[]).map(v=><button key={v} className={time===v?'active':''} onClick={()=>setTime(v)}>{v}</button>)}</div>
        ...
        {settingsOpen&&<div className="terminal-settings-pop"><label><span>Compact rows</span><input type="checkbox" checked={compactRows} onChange={e=>setCompactRows(e.target.checked)}/></label><label><span>Hide liquidity &lt; $10K</span><input type="checkbox" checked={hideLowLiquidity} onChange={e=>setHideLowLiquidity(e.target.checked)}/></label><small>1 / 2 / 3 changes PAPER buy preset.</small></div>}
      </section>
      ...
      <section className="market-workspace"><aside className="venue-rail">{venues.map(v=><button key={v.id} title={v.title} className={`${venue===v.id?'active ':''}${v.className||''}`} onClick={()=>setVenue(v.id)}>{v.label}</button>)}<div className="rail-divider"/><small>SOL</small></aside><div className="market-table-shell"><div className="market-table"><div className="terminal-table-head"><span>Pair Info</span><span>Chart</span><span>Market Cap</span><span>Liquidity</span><span>{time} Volume</span><span>{time} TXNS</span><span>Token Info</span><span>Action</span></div>{rows.map(t=>{
        const buys=buysFor(t,time),sells=sellsFor(t,time),tx=buys+sells,buyPct=Math.round(buys/Math.max(1,tx)*100),sellPct=100-buyPct,lqRatio=t.marketCap>0?Math.min(999,(t.liquidityUsd/t.marketCap)*100):0,change=changeFor(t,time),up=change>=0
      ...
      <div className={`terminal-source ${feedError?'warning':''}`} style={{margin:'8px 18px 0 78px'}}><i/>{feedError?'Feed degraded':`Live Solana · ${source||'market feed'} · real ${time} market snapshots`}</div>
