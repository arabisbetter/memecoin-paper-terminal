...
import { Eye, EyeOff, Globe2, Link2, Search, Settings2, SlidersHorizontal, Zap } from 'lucide-react'
...
export default function DiscoverHome(){
  ...
  useEffect(()=>{
    ...
    const handler=(event:Event)=>{
      ...
      if(detail){setPresetId(detail.id);setBuySize(detail.value)}
    }
    window.addEventListener('paper:preset',handler)
    return()=>window.removeEventListener('paper:preset',handler)
  },[])

  function choosePreset(id:string,value:number){
    setPresetId(id);setBuySize(value)
    window.localStorage.setItem('paper.quickBuyPreset',id)
    window.localStorage.setItem('paper.quickBuySize',String(value))
  }
  ...
  const rows=useMemo(()=>{
    ...
    if(tab==='radar'){
      const fresh=list.filter(t=>ageMs(t.pairCreatedAt)<4*60*60*1000)
      list=(fresh.length?fresh:list).sort((a,b)=>(b.pairCreatedAt||0)-(a.pairCreatedAt||0))
    }
  ...
  const venues:{id:VenueMode;label:string;className?:string;title:string}[]=[
    {id:'all',label:'All',title:'All Solana venues'},
    {id:'pump',label:'⚡',className:'venue-pump',title:'Pump venues'},
    {id:'raydium',label:'R',title:'Raydium'},
    {id:'meteora',label:'M',title:'Meteora'},
    {id:'other',label:'◇',title:'Other Solana venues'},
  ]
  ...
  return <div className="ax-app">
    ...
    <main className="market-console">
      <section className="market-topbar">
        <div className="market-tabs">{tabs.map(([id,label,isNew])=><button key={id} className={tab===id?'active':''} onClick={()=>setTab(id)}>{label}{isNew&&<span className="new-chip">NEW</span>}</button>)}</div>
        <div className="market-top-spacer"/>
        <div className="time-tabs-terminal">{(['1m','5m','30m','1h'] as Timeframe[]).map(v=><button key={v} className={time===v?'active':''} onClick={()=>setTime(v)}>{v}</button>)}</div>
        <div className="terminal-control-row">
          <button className={`icon-control ${settingsOpen?'active':''}`} title="Display settings" onClick={()=>setSettingsOpen(v=>!v)}><Settings2 size={15}/></button>
          <button className={`icon-control ${hideLowLiquidity?'active':''}`} title="Hide low-liquidity pairs" onClick={()=>setHideLowLiquidity(v=>!v)}>{hideLowLiquidity?<EyeOff size={15}/>:<Eye size={15}/>}</button>
          <div className="quick-buy-terminal"><Zap size={12} color="#6c82ff"/><span>Quick Buy</span><input aria-label="Quick buy PAPER SOL" type="number" min=".01" step=".01" value={buySize} onChange={e=>{setPresetId('');setBuySize(Math.max(.01,Number(e.target.value)||.01))}}/><small>PAPER SOL</small></div>
          <div className="preset-strip">{presets.map(p=><button key={p.id} className={presetId===p.id?'active':''} onClick={()=>choosePreset(p.id,p.value)}>{p.id}</button>)}</div>
        </div>
        {settingsOpen&&<div className="terminal-settings-pop"><label><span>Compact rows</span><input type="checkbox" checked={compactRows} onChange={e=>setCompactRows(e.target.checked)}/></label><label><span>Hide liquidity &lt; $10K</span><input type="checkbox" checked={hideLowLiquidity} onChange={e=>setHideLowLiquidity(e.target.checked)}/></label><label><span>Refresh every 10s</span><input type="checkbox" checked readOnly/></label></div>}
      </section>

      {feedError&&<div className="terminal-error"><span><b>Live feed:</b> {feedError}</span><button onClick={()=>void load()}>Retry</button></div>}
      {notice&&<div className="terminal-toast">{notice}</div>}

      <section className="market-workspace">
        <aside className="venue-rail">
          {venues.map(v=><button key={v.id} title={v.title} className={`${venue===v.id?'active ':''}${v.className||''}`} onClick={()=>setVenue(v.id)}>{v.label}</button>)}
          <div className="rail-divider"/><small>SOL</small>
        </aside>
        <div className="market-table-shell">
          <div className="market-table">
            <div className="terminal-table-head"><span>Pair Info</span><span>Chart</span><span>Market Cap</span><span>Liquidity</span><span>Volume</span><span>TXNS</span><span>Token Info</span><span>Action</span></div>
            {rows.map(t=>{
              ...
              const feeLike=Math.min(9.99,lqRatio/10)
              const up=t.priceChange24h>=0
              return <div className={`terminal-table-row ${compactRows?'compact':''}`} key={t.mint}>
                <button className="pair-cell-terminal" onClick={()=>router.push(`/spot?mint=${t.mint}`)}>
                  <div className="pair-avatar-terminal">{t.image?<img src={t.image} alt=""/>:<span>{t.symbol.slice(0,2)}</span>}</div>
                  <div className="pair-copy-terminal">
                    <div className="pair-title-terminal"><b>{t.symbol}</b><span>{t.name}</span><small className="pair-fee">◈ {feeLike.toFixed(2)}%</small></div>
                    <div className="pair-meta-terminal"><b>{age(t.pairCreatedAt)}</b><Link2 size={12}/><Globe2 size={12}/><Search size={12}/><Eye size={12}/><span>{tx.toLocaleString()}</span></div>
                  </div>
                </button>
                <div className="spark-cell-terminal"><MiniSparkline pool={t.pairAddress} timeframe={time}/></div>
                <div className="metric-terminal"><b>{money(t.marketCap)}</b><span className={up?'gain':'loss'}>{up?'+':''}{t.priceChange24h.toFixed(2)}%</span></div>
                <div className="metric-terminal"><b>{money(t.liquidityUsd)}</b><span>{lqRatio.toFixed(1)}% of MC</span></div>
                <div className="metric-terminal"><b>{money(t.volume24h)}</b><span>{t.dexId||'Solana'}</span></div>
                <div className="tx-terminal"><b>{tx.toLocaleString()}</b><span><em className="gain">{t.buys24h}</em> / <em className="loss">{t.sells24h}</em></span></div>
                <div className="token-info-terminal"><span className="metric-pill bad">S {sellPct}%</span><span className="metric-pill good">B {buyPct}%</span><span className="metric-pill">LQ {lqRatio.toFixed(1)}%</span><span className={`metric-pill ${up?'good':'bad'}`}>{up?'+':''}{Math.round(t.priceChange24h)}%</span><span className="metric-pill blue">{String(t.dexId||'SOL').slice(0,9)}</span><span className="metric-pill gold">{age(t.pairCreatedAt)}</span></div>
                <div className="terminal-buy-wrap"><button className="terminal-buy" disabled={buying===t.mint} onClick={()=>void quickBuy(t)}>{buying===t.mint?'Buying…':<>Buy {buySize}<small>PAPER SOL</small></>}</button></div>
              </div>
            })}
            {!rows.length&&loading&&<div className="terminal-empty">Loading live Solana memecoins…</div>}
            {!rows.length&&!loading&&<div className="terminal-empty">{feedError?'Live token feed is temporarily unavailable. Retrying automatically.':'No pairs match this filter right now.'}</div>}
          </div>
        </div>
      </section>
