...
export default function BottomDock({active}:{active:Active}){
  ...
  return <footer className="ax-bottom-dock">
    ...
    <div className="dock-divider"/><Link className={active==='spot'?'active':''} href="/"><Gauge size={14}/><span>Markets</span></Link><Link className={active==='pulse'?'active':''} href="/pulse"><Radio size={14}/><span>Pulse</span></Link><Link className={active==='portfolio'?'active':''} href="/portfolio"><BriefcaseBusiness size={14}/><span>Portfolio</span></Link><Link className={active==='leaderboard'?'active':''} href="/leaderboards"><BarChart3 size={14}/><span>Leaderboard</span></Link><Link className={active==='coin'?'active':''} href="/coin"><CircleDollarSign size={14}/><span>Coin</span></Link><Link className={active==='profile'?'active':''} href="/profile"><CircleUserRound size={14}/><span>Profile</span></Link><div className="dock-spacer"/><PaperAccountChip/><span className="dock-live"><i/> LIVE SOLANA</span><Link href="/profile" title="Settings"><Settings2 size={14}/></Link>
  </footer>
}
