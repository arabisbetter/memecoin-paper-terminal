...
import { BarChart3, BriefcaseBusiness, CircleDollarSign, CircleUserRound, Gift, Gauge, Layers3, Radio, Settings2, ShieldCheck, SlidersHorizontal, Star, Trophy, WalletCards } from 'lucide-react'
import PaperAccountChip from '@/components/PaperAccountChip'

type Active='discover'|'spot'|'evaluation'|'funded'|'pulse'|'portfolio'|'leaderboard'|'profile'|'coin'|'wallets'|'rewards'|'chains'|'watchlist'
const presetValues:[string,number][]=[['P1',0.1],['P2',0.5],['P3',1]]

export default function BottomDock({active}:{active:Active}){
  const pathname=usePathname()
  const currentActive:Active=pathname.startsWith('/discover')?'discover':pathname.startsWith('/spot')?'spot':pathname.startsWith('/evaluation')?'evaluation':pathname.startsWith('/funded')?'funded':pathname.startsWith('/pulse')?'pulse':pathname.startsWith('/portfolio')?'portfolio':pathname.startsWith('/leaderboards')?'leaderboard':pathname.startsWith('/profile')?'profile':pathname.startsWith('/coin')?'coin':pathname.startsWith('/wallets')?'wallets':pathname.startsWith('/rewards')?'rewards':pathname.startsWith('/chains')?'chains':pathname.startsWith('/watchlist')?'watchlist':active
