...
import { BarChart3, BriefcaseBusiness, CircleDollarSign, CircleUserRound, Gift, Gauge, Layers3, Radio, Settings2, SlidersHorizontal, Star, WalletCards } from 'lucide-react'
import PaperAccountChip from '@/components/PaperAccountChip'

type Active='spot'|'pulse'|'portfolio'|'leaderboard'|'profile'|'coin'|'wallets'|'rewards'|'chains'|'watchlist'
...
export default function BottomDock({active}:{active:Active}){
  ...
  return <footer className="ax-bottom-dock final-dock">
    ...
    <div className="dock-divider"/><Link className={active==='spot'?'active':''} href="/discover"><Gauge size={14}/><span>Markets</span></Link><Link className={active==='pulse'?'active':''} href="/pulse"><Radio size={14}/><span>Pulse</span></Link><Link className={active==='chains'?'active':''} href="/chains"><Layers3 size={14}/><span>Chains</span></Link><Link className={active==='portfolio'?'active':''} href="/portfolio"><BriefcaseBusiness size={14}/><span>Portfolio</span></Link><Link className={active==='wallets'?'active':''} href="/wallets"><WalletCards size={14}/><span>Wallets</span></Link><Link className={active==='watchlist'?'active':''} href="/watchlist"><Star size={14}/><span>Watchlist</span></Link><Link className={active==='rewards'?'active':''} href="/rewards"><Gift size={14}/><span>Rewards</span></Link><Link className={active==='leaderboard'?'active':''} href="/leaderboards"><BarChart3 size={14}/><span>Leaderboard</span></Link><Link className={active==='coin'?'active':''} href="/coin"><CircleDollarSign size={14}/><span>Coin</span></Link><Link className={active==='profile'?'active':''} href="/profile"><CircleUserRound size={14}/><span>Profile</span></Link><div className="dock-spacer"/><PaperAccountChip/><span className="dock-live"><i/> LIVE</span><Link href="/profile" title="Settings"><Settings2 size={14}/></Link>
  </footer>
}
