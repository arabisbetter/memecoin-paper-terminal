...
type Active='discover'|'spot'|'pulse'|'portfolio'|'leaderboard'|'profile'|'coin'|'wallets'|'rewards'|'chains'|'watchlist'
const presetValues:[string,number][]=[['P1',0.1],['P2',0.5],['P3',1]]

export default function BottomDock({active}:{active:Active}){
  const pathname=usePathname()
  const currentActive:Active=pathname.startsWith('/discover')?'discover':pathname.startsWith('/spot')?'spot':pathname.startsWith('/pulse')?'pulse':pathname.startsWith('/portfolio')?'portfolio':pathname.startsWith('/leaderboards')?'leaderboard':pathname.startsWith('/profile')?'profile':pathname.startsWith('/coin')?'coin':pathname.startsWith('/wallets')?'wallets':pathname.startsWith('/rewards')?'rewards':pathname.startsWith('/chains')?'chains':pathname.startsWith('/watchlist')?'watchlist':active
