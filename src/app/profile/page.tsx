...
export default function ProfilePage(){
  ...
  async function save(e:FormEvent){
    ...
    const {error}=await supabase.from('profiles').update({
      username:cleanUser,display_name:cleanDisplay,bio:cleanBio||null,x_handle:xHandle.trim()||null,
      avatar_emoji:avatar,accent,updated_at:new Date().toISOString(),
    }).eq('id',userId)
  ...
  async function linkPayoutWallet(){
    ...
    try{
      ...
      setSecurity({payout_wallet_address:walletAddress,flagged_for_review:false})
      setPayout({...payout,status:'ready',payout_wallet_snapshot:walletAddress})
      setPayoutMessage('Payout address verified. This signature did not authorize any trade or transfer.')
    }catch(e){setPayoutMessage(e instanceof Error?e.message:'Payout wallet verification failed')}
    finally{setLinking(false)}
  }

  return <div className="page-shell">
    <header className="page-top"><Link href="/" className="brand">PAPER</Link><div className="spacer"/><Link href="/">Spot</Link><Link href="/pulse">Pulse</Link></header>
    <main className="page-content narrow">
      <div className="eyebrow">PAPER TRADER PROFILE</div>
      <h1>Your trading identity.</h1>
      <p className="lead">Your PAPER account is automatic. A real Solana address is never needed to trade; it only appears here if you actually have a prize waiting.</p>
      {busy?<div className="empty-card">Loading your PAPER profile…</div>:!profile?<div className="error-card">{message||'Profile unavailable.'}</div>:<>
        <form className="profile-card" onSubmit={save}>
          <div className="profile-preview">
            <div className={`preview-avatar accent-${accent}`}>{avatarUrl?<img src={avatarUrl} alt="Profile"/>:avatar}</div>
            <div><strong>{displayName||'Paper Trader'}</strong><div className="muted-small">@{username||'paper_trader'}</div><div className="muted-small">{Number(profile.paper_cash_sol).toFixed(2)} PAPER SOL balance</div></div>
          </div>
          ...
          <div className="profile-grid">
            ...
            <label>PAPER balance<input value={`${Number(profile.paper_cash_sol).toFixed(2)} PAPER SOL`} disabled/></label>
          </div>
          <label style={{marginTop:16}}>Default avatar</label>
        ...
        <section className="account-panel">
          <div><div className="eyebrow">ACCOUNT RECOVERY</div><h3>Protect your PAPER history</h3><p>Add an email so clearing cookies or changing devices does not have to mean losing this account. This is optional.</p></div>
          <div className="inline-action"><input type="email" value={recoveryEmail} placeholder="you@example.com" onChange={e=>setRecoveryEmail(e.target.value)}/><button onClick={()=>void addRecoveryEmail()}>ADD RECOVERY EMAIL</button></div>
          {recoveryMessage&&<div className="profile-message">{recoveryMessage}</div>}
        </section>

        {payout&&<section className="account-panel payout-panel">
          <div className="eyebrow">REAL SOL PRIZE</div>
          <h3>You’re in line for a payout.</h3>
          <p><b>{Number(payout.prize_sol).toFixed(3)} REAL SOL</b> · {payout.category} · week of {payout.week_start}. Your PAPER trading account remains separate from the payout address.</p>
          {security?.flagged_for_review&&<div className="onboard-error">This account is under payout review.</div>}
          {security?.payout_wallet_address?<div className="verified-wallet"><span>Verified payout address</span><b>{security.payout_wallet_address.slice(0,6)}…{security.payout_wallet_address.slice(-6)}</b></div>:<button className="profile-save" disabled={linking||security?.flagged_for_review} onClick={()=>void linkPayoutWallet()}>{linking?'VERIFYING…':'LINK SOLANA PAYOUT ADDRESS'}</button>}
          <small>This one-time signature only proves control of the receiving address. It cannot trade or move funds.</small>
          {payoutMessage&&<div className="profile-message">{payoutMessage}</div>}
        </section>}
      </>}
    </main>
  </div>
