...
type StatusPayload={ok?:boolean;anonymous?:boolean;hasEvaluation?:boolean;evaluation?:Evaluation|null;funded?:{stage?:string;kyc_status?:string}|null;platform?:{real_funded_activation?:boolean;real_payouts_enabled?:boolean};rules?:Record<string,number|string>;error?:string}
...
export default function EvaluationPage(){
  ...
  return <div className="ax-app"><AppHeader active="evaluation"/><main className="terminal-page evaluation-page"><div className="terminal-page-inner evaluation-inner">
    <section className="eval-hero"><div><div className="terminal-eyebrow">PAPER EVALUATION · FREE · REAL MARKET DATA</div><h1>Prove it with PAPER.</h1><p className="terminal-lead">Start with $1,000 PAPER. Reach $6,000 live equity without breaking the risk rules. Passing unlocks the funded-account qualification stage — real capital remains disabled until KYC and compliance are active.</p></div><div className={`eval-status ${active?'live':passed?'passed':ended?'failed':''}`}><i/>{loading?'SYNCING':active?'EVALUATION LIVE':passed?'PASSED':ended?status.toUpperCase():'READY'}</div></section>
