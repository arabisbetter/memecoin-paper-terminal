...
export default function PortfolioPage(){
  ...
  const pnl24h=useMemo(()=>{const cutoff=Date.now()-86400000;const snap=snapshots.find(s=>new Date(s.created_at).getTime()>=cutoff)||snapshots[0];return snap?equity-Number(snap.equity_usd):pnl},[snapshots,equity,pnl])
  const pnl7d=useMemo(()=>{const cutoff=Date.now()-7*86400000;const snap=snapshots.find(s=>new Date(s.created_at).getTime()>=cutoff)||snapshots[0];return snap?equity-Number(snap.equity_usd):pnl},[snapshots,equity,pnl])
