...
export default function PortfolioPage(){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      const sj=await solRes.json().catch(()=>null);if(solRes.ok)setSolUsd(Number(sj?.priceUsd||0));setNowMs(Date.now());setError('')
    }catch(e){setError(e instanceof Error?e.message:'Portfolio unavailable')}finally{setLoading(false)}
  },[supabase])

  useEffect(()=>{const start=window.setTimeout(()=>void load(),0),id=window.setInterval(()=>{if(!document.hidden)void load()},10000),changed=()=>void load();window.addEventListener('paper:account-changed',changed);return()=>{clearTimeout(start);clearInterval(id);window.removeEventListener('paper:account-changed',changed)}},[load])
  ...
  const winners=closed.filter(p=>Number(p.realized_pnl_usd||0)>0),losers=closed.filter(p=>Number(p.realized_pnl_usd||0)<0),winRate=closed.length?winners.length/closed.length*100:0,avgWinner=winners.length?winners.reduce((s,p)=>s+Number(p.realized_pnl_usd||0),0)/winners.length:0,avgLoser=losers.length?losers.reduce((s,p)=>s+Number(p.realized_pnl_usd||0),0)/losers.length:0,bestTrade=closed.length?Math.max(...closed.map(p=>Number(p.realized_pnl_usd||0))):0,worstTrade=closed.length?Math.min(...closed.map(p=>Number(p.realized_pnl_usd||0))):0,grossProfit=winners.reduce((s,p)=>s+Number(p.realized_pnl_usd||0),0),grossLoss=Math.abs(losers.reduce((s,p)=>s+Number(p.realized_pnl_usd||0),0)),profitFactor=grossLoss>0?grossProfit/grossLoss:grossProfit>0?99:0,avgHoldHours=closed.length?closed.reduce((s,p)=>s+Math.max(0,(new Date(p.closed_at||p.opened_at).getTime()-new Date(p.opened_at).getTime())/3600000),0)/closed.length:0
  const equityPoints=useMemo(()=>{const list=snapshots.map(s=>({time:Math.floor(new Date(s.created_at).getTime()/1000),equity:Number(s.equity_usd)}));if(account?.created_at){const t=Math.floor(new Date(account.created_at).getTime()/1000);if(Number.isFinite(t)&&(!list.length||t<list[0].time))list.unshift({time:t,equity:initial})}const now=Math.floor(nowMs/1000);if(now>0&&(!list.length||Math.abs((list[list.length-1]?.equity||0)-equity)>.001||now-(list[list.length-1]?.time||0)>90))list.push({time:now,equity});return list},[snapshots,account,initial,equity,nowMs])
  const maxDrawdown=useMemo(()=>{let peak=0,max=0;for(const p of equityPoints){peak=Math.max(peak,p.equity);if(peak>0)max=Math.max(max,(peak-p.equity)/peak*100)}return max},[equityPoints])
  const pnl24h=useMemo(()=>{const cutoff=nowMs-86400000;const snap=snapshots.find(s=>new Date(s.created_at).getTime()>=cutoff)||snapshots[0];return snap?equity-Number(snap.equity_usd):pnl},[snapshots,equity,pnl,nowMs])
  const pnl7d=useMemo(()=>{const cutoff=nowMs-7*86400000;const snap=snapshots.find(s=>new Date(s.created_at).getTime()>=cutoff)||snapshots[0];return snap?equity-Number(snap.equity_usd):pnl},[snapshots,equity,pnl,nowMs])
  ...
  const performance=[['TOTAL P&L',signedMoney(pnl),pnl],['24H P&L',signedMoney(pnl24h),pnl24h],['7D P&L',signedMoney(pnl7d),pnl7d],['REALIZED',signedMoney(realized),realized],['UNREALIZED',signedMoney(unrealized),unrealized],['TXNS',tradeCount.toLocaleString(),0],['WIN RATE',`${winRate.toFixed(1)}%`,winRate-50],['PROFIT FACTOR',profitFactor?profitFactor.toFixed(2):'—',profitFactor-1],['MAX DRAWDOWN',`${maxDrawdown.toFixed(2)}%`,-maxDrawdown],['AVG HOLD',closed.length?`${avgHoldHours.toFixed(1)}h`:'—',0],['AVG WIN',winners.length?signedMoney(avgWinner):'—',avgWinner],['AVG LOSS',losers.length?signedMoney(avgLoser):'—',avgLoser],['BEST CLOSE',closed.length?signedMoney(bestTrade):'—',bestTrade],['WORST CLOSE',closed.length?signedMoney(worstTrade):'—',worstTrade]] as const
  const pnlCalendar=useMemo(()=>{const by=new Map<string,number>();for(const s of snapshots){const d=new Date(s.created_at).toISOString().slice(0,10);by.set(d,Number(s.equity_usd))}const days=[...by.entries()].sort((a,b)=>a[0].localeCompare(b[0]));return days.map(([date,value],i)=>({date,value,pnl:i?value-days[i-1][1]:value-initial})).slice(-28)},[snapshots,initial])
  const achievements=useMemo(()=>[
    {label:'First fill',unlocked:tradeCount>=1,detail:'Complete a PAPER trade'},
    {label:'Active trader',unlocked:tradeCount>=25,detail:'25 PAPER fills'},
    {label:'Century',unlocked:tradeCount>=100,detail:'100 PAPER fills'},
    {label:'Green account',unlocked:pnl>0,detail:'Positive total P&L'},
    {label:'Disciplined',unlocked:tradeCount>=20&&maxDrawdown<5,detail:'20+ fills with <5% max drawdown'},
    {label:'Edge found',unlocked:closed.length>=10&&profitFactor>=1.5,detail:'Profit factor ≥1.50 over 10+ closes'},
    {label:'Consistency',unlocked:closed.length>=10&&winRate>=60,detail:'60%+ win rate over 10+ closes'},
  ],[tradeCount,pnl,maxDrawdown,closed.length,profitFactor,winRate])
  async function sharePerformance(){const text=`PAPER · ${signedMoney(pnl)} total P&L · ${roi>=0?'+':''}${roi.toFixed(2)}% ROI · ${winRate.toFixed(1)}% win rate · ${tradeCount} fills`;try{if(navigator.share)await navigator.share({title:'My PAPER trading stats',text,url:location.origin+'/portfolio'});else await navigator.clipboard.writeText(text+' · '+location.origin+'/portfolio')}catch{}}
  ...
  return <div className="ax-app"><AppHeader active="portfolio"/><main className="terminal-page portfolio-page p45-portfolio"><div className="terminal-page-inner portfolio-inner">
    ...
    <section className="portfolio-pro-grid"><article className="portfolio-calendar-card"><div className="portfolio-panel-head"><div><b>P&L CALENDAR</b><span>Latest snapshot-to-snapshot daily equity change</span></div><button onClick={()=>void sharePerformance()}>SHARE STATS</button></div><div className="pnl-calendar">{pnlCalendar.length?pnlCalendar.map(d=><div key={d.date} className={d.pnl>0?'gain':d.pnl<0?'loss':'flat'} title={d.date+' · '+signedMoney(d.pnl)}><small>{d.date.slice(5)}</small><b>{d.pnl===0?'—':signedMoney(d.pnl)}</b></div>):<div className="portfolio-empty">Daily P&L appears as portfolio snapshots accumulate.</div>}</div></article><article className="portfolio-achievement-card"><div className="portfolio-panel-head"><div><b>ACHIEVEMENTS</b><span>Automatically unlocked from PAPER activity</span></div><span>{achievements.filter(a=>a.unlocked).length}/{achievements.length}</span></div><div className="achievement-grid">{achievements.map(a=><div key={a.label} className={a.unlocked?'unlocked':'locked'}><b>{a.label}</b><small>{a.detail}</small></div>)}</div></article></section>
