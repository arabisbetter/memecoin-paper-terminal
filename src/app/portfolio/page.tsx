...
export default function PortfolioPage(){
  ...
  useEffect(()=>{if(!supabase||!userId||!account||loading||snapshotBusy.current||!Number.isFinite(equity))return;snapshotBusy.current=true;void supabase.rpc('record_portfolio_snapshot',{p_equity_usd:equity,p_cash_usd:cash,p_open_value_usd:openValue,p_realized_pnl_usd:realized,p_unrealized_pnl_usd:unrealized}).then(({error})=>{if(!error){const now=new Date().toISOString();setSnapshots(cur=>{const last=cur[cur.length-1];if(last&&Date.now()-new Date(last.created_at).getTime()<55_000)return cur;return[...cur,{id:Date.now(),equity_usd:equity,cash_usd:cash,open_value_usd:openValue,realized_pnl_usd:realized,unrealized_pnl_usd:unrealized,created_at:now}].slice(-1000)})}}).finally(()=>{snapshotBusy.current=false})},[supabase,userId,account,loading,equity,cash,openValue,realized,unrealized])
