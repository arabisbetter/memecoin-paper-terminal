...
export default function ComparePage(){
  ...
  return <div className="ax-app"><AppHeader active="spot"/><main className="terminal-page"><div className="terminal-page-inner"><div className="final-page-hero"><div><div className="terminal-eyebrow">TOKEN COMPARISON</div><h1>Compare up to four markets.</h1><p className="terminal-lead">Liquidity, volume, momentum, buy pressure and PAPER risk scans side by side.</p></div><GitCompareArrows size={28}/></div><form className="compare-add" onSubmit={add}><input value={input} onChange={e=>setInput(e.target.value)} placeholder="Paste Solana contract address"/><button disabled={tokens.length>=4}><Plus size={13}/> Add</button></form>{error&&<div className="error-card">{error}</div>}<section className="compare-grid">{tokens.map(t=>{const buys=Number(t.buys5m||0),sells=Number(t.sells5m||0),buyPct=buys/Math.max(1,buys+sells)*100;return <article className="compare-card" key={t.mint}><button className="compare-remove" onClick={()=>setTokens(v=>v.filter(x=>x.mint!==t.mint))}><X size={13}/></button><div className="compare-token"><b>{'$'+t.symbol}</b><span>{t.name}</span><Link href={'/spot?mint='+t.mint}>Open terminal →</Link></div><div className="compare-kpis"><span><small>MARKET CAP</small><b>{money(t.marketCap)}</b></span><span><small>LIQUIDITY</small><b>{money(t.liquidityUsd)}</b></span><span><small>5M VOLUME</small><b>{money(Number(t.volume5m||0))}</b></span><span><small>5M MOVE</small><b className={Number(t.priceChange5m||0)>=0?'gain':'loss'}>{Number(t.priceChange5m||0).toFixed(1)}%</b></span><span><small>BUY PRESSURE</small><b>{buyPct.toFixed(0)}%</b></span><span><small>AGE</small><b>{t.pairCreatedAt?Math.max(0,(now-t.pairCreatedAt)/3600000).toFixed(1)+'h':'—'}</b></span></div><TokenRiskPanel mint={t.mint}/></article>})}{!tokens.length&&<div className="empty-card">Add 2–4 token contract addresses to compare them.</div>}</section></div></main><BottomDock active="spot"/></div>
}
