...
export default function WorkspacesPage(){
  ...
  async function setMint(i:number,mint:string){setSlots(v=>v.map((s,x)=>x===i?{...s,mint}:s));if(!mint.trim())return;try{const r=await fetch('/api/market/token/'+encodeURIComponent(mint.trim()),{cache:'no-store'}),j=await r.json();if(!r.ok||!j.token)throw new Error(j.error||'Token not found');setSlots(v=>v.map((s,x)=>x===i?{mint:j.token.mint,token:j.token}:s));setError('')}catch(e){setError(e instanceof Error?e.message:'Token not found')}}
