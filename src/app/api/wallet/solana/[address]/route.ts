...
const TOKEN_PROGRAM='TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA',valid=/^[1-9A-HJ-NP-Za-km-z]{32,44}$/
...
async function rpc<T>(method:string,params:unknown[]):Promise<T>{
  ...
  const tasks=RPCS.map(async(endpoint,i)=>{
    const timer=setTimeout(()=>controllers[i].abort(),8000)
    try{
  ...
  try{
    ...
    throw error instanceof Error?error:new Error('All free Solana RPC endpoints failed')
  }
}
