...
const RPC='https://api.mainnet-beta.solana.com',TOKEN_PROGRAM='TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA',valid=/^[1-9A-HJ-NP-Za-km-z]{32,44}$/
...
async function rpc<T>(method:string,params:unknown[]):Promise<T>{const c=new AbortController(),timer=setTimeout(()=>c.abort(),8000);try{const r=await fetch(RPC,{method:'POST',cache:'no-store',signal:c.signal,headers:{'content-type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id:1,method,params})});if(!r.ok)throw new Error(`Solana RPC ${r.status}`);const body=await r.json() as RpcResult<T>;if(body.error)throw new Error(body.error.message||'Solana RPC error');if(body.result===undefined)throw new Error('Solana RPC returned no result');return body.result}finally{clearTimeout(timer)}}
async function dexMetadata(mints:string[]){const out=new Map<string,{symbol?:string;name?:string;image?:string;priceUsd:number;valueSource?:string}>();for(const group of chunks(mints.slice(0,90),30)){try{const r=await fetch(`https://api.dexscreener.com/tokens/v1/solana/${group.join(',')}`,{cache:'no-store',headers:{Accept:'application/json'}});if(!r.ok)continue;const pairs=await r.json() as DexPair[];for(const p of pairs){if(p.chainId!=='solana'||!p.baseToken?.address)continue;const key=p.baseToken.address,prev=out.get(key),liq=Number(p.liquidity?.usd||0),prevLiq=Number((prev as {liquidity?:number}|undefined)?.liquidity||0);if(!prev||liq>prevLiq)out.set(key,{symbol:p.baseToken.symbol,name:p.baseToken.name,image:p.info?.imageUrl,priceUsd:Number(p.priceUsd||0),valueSource:'dexscreener',liquidity:liq} as never)}}catch{/* metadata is best effort */}}return out}
