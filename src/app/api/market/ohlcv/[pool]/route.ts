...
const validTimeframes=new Map([
  ['1m',['minute','1']],['5m',['minute','5']],['15m',['minute','15']],['30m',['hour','1']],['1h',['hour','1']],['1M',['day','1']],
] as const)
...
export async function GET(req:NextRequest,ctx:{params:Promise<{pool:string}>}){
  ...
  const config=validTimeframes.get(tf as '1m'|'5m'|'15m'|'30m'|'1h'|'1M')||validTimeframes.get('1m')!
  const key=`${pool}:${tf}`
  const previous=cache.get(key)
  const cacheMs=tf==='1M'?120_000:35_000
  ...
  try{
    ...
    url.searchParams.set('aggregate',aggregate);url.searchParams.set('limit',tf==='1M'?'35':'180');url.searchParams.set('currency','usd');url.searchParams.set('token','base');url.searchParams.set('include_empty_intervals','false')
    ...
    return NextResponse.json({candles,timeframe:tf,source:'geckoterminal',live:true,asOf:Date.now()},{headers:{'Cache-Control':tf==='1M'?'public, s-maxage=60, stale-while-revalidate=300':'public, s-maxage=20, stale-while-revalidate=90'}})
  }catch(error){
