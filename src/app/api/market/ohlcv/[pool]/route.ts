...
type Config={unit:'minute'|'hour'|'day';aggregate:string;limit:number;bucketSeconds?:number}
const configs:Record<string,Config>={
  '1m':{unit:'minute',aggregate:'1',limit:180},
  '5m':{unit:'minute',aggregate:'5',limit:180},
  '15m':{unit:'minute',aggregate:'15',limit:180},
  '30m':{unit:'minute',aggregate:'15',limit:360,bucketSeconds:1800},
  '1h':{unit:'hour',aggregate:'1',limit:180},
  '4h':{unit:'hour',aggregate:'4',limit:180},
  '1d':{unit:'day',aggregate:'1',limit:180},
  '1M':{unit:'day',aggregate:'1',limit:35},
}
const cache=new Map<string,{at:number;candles:Candle[]}>()

function aggregateCandles(rows:Candle[],bucketSeconds:number){
  const groups=new Map<number,Candle[]>()
  for(const row of rows){const bucket=Math.floor(row.time/bucketSeconds)*bucketSeconds;const list=groups.get(bucket)||[];list.push(row);groups.set(bucket,list)}
...
export async function GET(req:NextRequest,ctx:{params:Promise<{pool:string}>}){
  ...
  const requested=req.nextUrl.searchParams.get('tf')||'1m',tf=configs[requested]?requested:'1m',config=configs[tf],key=`${pool}:${tf}`,previous=cache.get(key),cacheMs=tf==='1M'||tf==='1d'?120_000:35_000
  if(previous&&Date.now()-previous.at<cacheMs)return NextResponse.json({candles:previous.candles,timeframe:tf,source:'geckoterminal',live:true,cached:true,asOf:previous.at})
  ...
  try{
    ...
    return NextResponse.json({candles,timeframe:tf,source:'geckoterminal',live:true,asOf:at},{headers:{'Cache-Control':tf==='1M'||tf==='1d'?'public, s-maxage=60, stale-while-revalidate=300':'public, s-maxage=20, stale-while-revalidate=90'}})
  }catch(error){
