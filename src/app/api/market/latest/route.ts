...
type DiscoveryItem={chainId?:string;tokenAddress?:string;icon?:string}
...
async function dexScreenerFeed():Promise<MarketToken[]>{
  ...
  for(const result of settled){
    ...
    for(const item of result.value){
      ...
      if(item.icon)icons.set(item.tokenAddress,item.icon)
    }
  }
  ...
  for(const raw of pairResponses.flat()){
    ...
    if(!token.image){const icon=icons.get(token.mint);if(icon)token.image=icon}
...
export async function GET(){
  const now=Date.now()
  if(lastGood&&now-lastGood.at<CACHE_MS)return NextResponse.json({tokens:lastGood.tokens,source:lastGood.source,live:true,cached:true,asOf:lastGood.at})

  const errors:string[]=[]
  try{
    const tokens=await dexScreenerFeed()
    lastGood={at:now,tokens,source:'dexscreener'}
    return NextResponse.json({tokens,source:'dexscreener',live:true,asOf:now},{headers:{'Cache-Control':'public, s-maxage=10, stale-while-revalidate=30'}})
  }catch(error){
  ...
  try{
    ...
    return NextResponse.json({tokens,source:'geckoterminal',live:true,fallback:true,asOf:now},{headers:{'Cache-Control':'public, s-maxage=20, stale-while-revalidate=60'}})
  }catch(error){
