...
function PulseCard({t,compact,buySize,buying,disabled,instantMode,viewCount,onOpen,onAction}:{t:MarketToken;compact:boolean;buySize:number;buying:boolean;disabled:boolean;instantMode:boolean;viewCount:number;onOpen:()=>void;onAction:()=>void}){
  ...
  return <div className={`pulse-card-terminal ${compact?'compact':''}`} role="button" tabIndex={0} onClick={onOpen} aria-label={'Open '+t.symbol+' in Trade'} onKeyDown={e=>{if(e.target!==e.currentTarget)return;if(e.key==='Enter'||e.key===' '){e.preventDefault();onOpen()}}}>
    ...
    <div className="pulse-card-body"><div className="pulse-card-title"><b>{t.symbol}</b><span>{t.name}</span><i>MC <strong>{money(t.marketCap)}</strong></i></div><div className="pulse-card-meta"><em>{age(t.pairCreatedAt)}</em><span className="pulse-live-dot">LIVE</span><span>5m V {money(Number(t.volume5m||0))}</span><span className="pulse-view-count" title="Unique active viewers"><Eye size={9}/>{viewCount.toLocaleString()} viewers</span><span>◎ {tx}</span><span>≋ {t.priceNative?t.priceNative.toFixed(4):'—'}</span></div><div className="pulse-card-badges"><span className={buyPct>=50?'metric-pill good':'metric-pill bad'}>B {buyPct}%</span><span className="metric-pill">LQ {lqRatio.toFixed(1)}%</span><span className={liveChange>=0?'metric-pill good':'metric-pill bad'}>{liveChange>=0?'+':''}{liveChange.toFixed(1)}%</span><span className="metric-pill blue">{String(t.dexId||'SOL').slice(0,8)}</span></div></div>
...
export default function PulsePage(){
  ...
  const loadBusy=useRef(false),manualRefreshQueued=useRef(false),buyLock=useRef(false),router=useRouter(),supabase=useMemo(()=>{try{return createClient()}catch{return null}},[]),[instantMode]=useInstantMode(),filterCount=activeFilterCount(filters)
  const viewerStats=useTokenViewers(tokens.map(t=>t.mint),5000)
  const views=viewerStats.byMint

  async function load(force=false,manual=false){
    if(document.hidden&&!force)return
    if(loadBusy.current){if(manual){manualRefreshQueued.current=true;setRefreshing(true)}return}
    loadBusy.current=true
    if(force)setRefreshing(true)
    try{
      const endpoint=manual?`/api/market/latest?refresh=${Date.now()}`:'/api/market/latest'
      const r=await fetch(endpoint,{cache:'no-store'}),j=await r.json()
      ...
      setTokens(next);setSource(j.source||'market feed');setFeedError('');setDataStatus(j.stale?'STALE':j.warning||j.fallback||j.live===false?'DEGRADED':'LIVE')
      if(j.warning)setFeedError(j.warning)
      if(manual)setNotice('Pulse refreshed from the live market feed.')
    }catch(e){
      setFeedError(e instanceof Error?e.message:'Live market feed unavailable');setDataStatus('DEGRADED')
      if(manual)setNotice('Pulse refresh failed. Retrying automatically.')
    }finally{
      loadBusy.current=false
      const rerun=manualRefreshQueued.current
      manualRefreshQueued.current=false
      if(force&&!rerun)setRefreshing(false)
      if(rerun)void load(true,true)
    }
  }
