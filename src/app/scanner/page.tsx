...
export default function ScannerPage(){
  ...
  const [deep,setDeep]=useState<Record<string,Deep>>({}),[deepBusy,setDeepBusy]=useState(false),[deepAt,setDeepAt]=useState(0),[marketAt,setMarketAt]=useState(0)
  const loadBusy=useRef(false),alive=useRef(true)
  async function load(){if(loadBusy.current)return;loadBusy.current=true;setLoading(true);try{const r=await fetch('/api/market/latest',{cache:'no-store'}),j=await r.json();if(!r.ok)throw new Error(j.error||'Market feed unavailable');if(alive.current){setTokens(j.tokens||[]);setMarketAt(Date.now());setError('')}}catch(e){if(alive.current)setError(e instanceof Error?e.message:'Market feed unavailable')}finally{loadBusy.current=false;if(alive.current)setLoading(false)}}
  ...
  useEffect(()=>{alive.current=true;const start=window.setTimeout(()=>{void load();void loadSaved()},0),id=setInterval(()=>{if(!document.hidden)void load()},4000);return()=>{alive.current=false;clearTimeout(start);clearInterval(id)}},[])

  const baseRows=useMemo(()=>tokens.map(t=>{const buys=Number(t.buys5m||0),sells=Number(t.sells5m||0),buyPct=buys/Math.max(1,buys+sells)*100,ageMin=t.pairCreatedAt&&marketAt>0?(marketAt-t.pairCreatedAt)/60000:999999,liqMcPct=t.marketCap>0?t.liquidityUsd/t.marketCap*100:0;const velocity=Number(t.volume5m||0)/5+buys*200+Math.max(0,Number(t.priceChange5m||0))*800;return{...t,buyPct,ageMin,liqMcPct,velocity}}).filter(t=>t.ageMin<=filters.maxAgeMin&&t.liquidityUsd>=filters.minLiquidity&&t.marketCap<=filters.maxMarketCap&&Number(t.volume5m||0)>=filters.minVolume5m&&Number(t.buys5m||0)>=filters.minBuys5m&&t.buyPct>=filters.minBuyPct&&Number(t.priceChange5m||0)>=filters.minMove5m&&t.liqMcPct>=filters.minLiqMcPct).sort((a,b)=>b.velocity-a.velocity),[tokens,filters,marketAt])
