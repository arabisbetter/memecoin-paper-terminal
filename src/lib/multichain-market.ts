...
async function profiles(){const urls=['https://api.dexscreener.com/token-profiles/latest/v1','https://api.dexscreener.com/token-boosts/latest/v1','https://api.dexscreener.com/token-boosts/top/v1','https://api.dexscreener.com/community-takeovers/latest/v1'];const settled=await Promise.allSettled(urls.map(u=>fetchJson<Profile[]>(u)));return settled.flatMap(r=>r.status==='fulfilled'&&Array.isArray(r.value)?r.value:[])}
