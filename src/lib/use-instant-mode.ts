...
export function useInstantMode(){
  ...
  useEffect(()=>{
    setInstantState(localStorage.getItem('paper.instantMode')==='1')
    ...
    return()=>{window.removeEventListener('paper:instant-mode',onChange);window.removeEventListener('storage',onStorage)}
  },[])
